...
public class ScoreServiceImplV1A implements ScoreService{
	...
	/*
	 * 학번 입력하면 scoreList에 같은 학번의 성적이 있는지 확인하여 
	 * 있으면 저장이 안되게
	...
	// 파라메터(매개변수) 로 전달받은 학번이 scoreList에 있는지 검사
	// 있으면 scoreVO return하고 
	// 없으면 null을 return;
	...
	public void printScore() {
		...
		System.out.println("-".repeat(50));
		ScoreVO cVO = scoreList.get(0);
		StudentVO tVO = new StudentVO();
		
		  
		System.out.println(cVO.getNum() + "\t");
		System.out.print(tVO.getName() + "\t");
		System.out.print(tVO.getDept() + "\t");
		System.out.print(tVO.getGrade() + "\t");
		System.out.print(cVO.getKor() + "\t");
		System.out.print(cVO.getEng() + "\t");
		System.out.print(cVO.getMath() + "\t");
		System.out.print(cVO.getTotal() + "\t");
		System.out.println(cVO.getAvg());
