 ...
 */
	 ...
	 * 몇명의 학생, 일부과목 점수만 입력마치고 잠시 업무를 멈추고 싶을때 현재 코드는 불가능하다
	...
	public void makeScores() {
		...
		for(int i = 0; i < intKor.length; i++) {
			...
			for(int j = 0; j < subject.length; j++) {
				...
				/*
				 * 입력업무중에 -1을 입력했을때는 입력업무를 중단하고 main() 으로 되돌아가고싶다.
				 * 이때 break;를 사용하면 for()가 겹쳐있기때문에 원하는대로 코드가 작동되지 않는다. 
	...
	private Integer inputScore(String sub) {
		...
		while(true) {
			...
			System.out.print(">> ");
			score = scan.nextInt();
			//점수입력을 하는 중에 -1을 입력하면 입력하는 업무를 중단하자
			if(score == -1) {
				return -1;
				...
				// continue 명령을 사용하는 대신 else를 사용하는 코드를 작성
			}else {
				break;
			}
		}
