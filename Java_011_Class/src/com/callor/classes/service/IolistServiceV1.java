 ...
 * Java에서는 배열을 사용해야 하는 프로젝트에서 List클래스를 사용한 객체를 생성하여 진행을 한다.
 */
public class IolistServiceV1 {
	...
	// 낱개 데이터는 IolistVO 클래스 형으로 사용하고 그 데이터들을 묶어서 List형으로 연산을 수행하겠다 선언

	// iolist를 private으로 선언한 이유
	// 혹시 외부에서 iolist데이터에 접근하여 값을 저장하는 일을 방지하기 위함이다.
	...
	// return type이 명확히 결정되지 않았을때는 void형으로 작성을 한다.
