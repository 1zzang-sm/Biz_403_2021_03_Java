...
/*
 * 1. InputServiceV1 class의 클래스 영역에 List<Integer> intList 리스트 객체 선언.
 * 2. 클래스 생성자에서 intList, scan 객체 생성(초기화)
 * 3. inputNum() method를 선언하고, 키보드로 정수를 입력 받아, intList 객체에 추가(add)하는 코드작성
 * 4. Prompt 등을 적절히 추가하여 작성
 * 5. printNum() method를 선언하고, 입력된 리스트를 출력하는 코드 작성
 */
...
public class InputServiceV1 {
	//접근 제한자 - 나 혼자 단독으로 사용할때는 private
	// protected : 이 클래스를 누군가 상속하여 사용할 수 있도록 하기 위해서
	// 정수형 데이터들을 담을 리스트형 객체 Integer wrapper 클래스 형 데이터들을 담을 리스트
	...
	//반드시 생성(초기화)를 해 주어야 한다. 현재 Service 클래스의 생성자 method에서 수행을 한다.
	...
	// { } : 블래이드, 코드블럭 
	// 코드블럭( { } )에 연산을 수행하는 코드를 작성하고 그 코드들의 묶음을 inputNum라고 하겠다 라는 선언
