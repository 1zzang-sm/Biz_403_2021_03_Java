...
/*함수형 프로그래밍 언어( 또는 구조적 프로그래밍 언어) 에서는 C 
 * 	한개의 모듈(파일) 또는 한 프로젝트에서 같은 이름의 함수를 선언할 수 없다.
 * 
 * 객체지향 프로그래밍언어(java)에서는 한개의 클래스(파일)내에서 같은 이름의 함수(method)를
 * 중복하여 선언할 수 있다.
 * 		한개의 클래스에서 method를 중복선언 할 수 있다.*/
  
 // 단, 조건이 있는데 매개변수가 달라야한다.
 ...
 * 		==> 누구나 어디선가 addNum()를 호출할때는 객체.addNum() 2개의 정수를 전달하게 하겠다.
 * 			객체.addNum(100, 200)
 * 	두개의 정수를 전달받기 위해서는 method가 준비를 해야한다.
 * 		==>method의 () 안에 변수를 선언한다. 두개의 변수를 선언하자 
 * 
 * 3. 매개변수로 받은 2개의 정수의 합을 계산하고 합의 결과가 짝수이면 계산 결과를 return하고 
 * 			그렇지 않으면 null을 return한다.
 * 
 * return 값 형식의 명령이 있으려면 void가 아닌 return type을 지정해 주어야 한다.
 * 	단순히 정수만 return하는 경우는 int type으로 하면 되는데 
 * 여기에서는 어떤 조건에 따라 null값을 return 해야하는 경우도 있기때문에
 * int type 이 아니고 Integer type으로 설정해야 한다. 
 */
public class NumberServiceV1 {
	// 첫번째 방법
	/* 어떤 연산을 수행하고 연산결과에 따라 return등을 수행해야 하는 경우
	...
	public Integer addNum(int num1, int num2) {
		...
		if (addNum % 2 == 0) {
			...
			return null;
		} // else가 굳이 안와도 된다. if문을 끝내고 return null 해도 된다.
	}//addNum(int num1, int num2) 끝나는 곳 
	
	// 두번째방법 
	/* ( C언어와 같은 함수형 언어에서는 함수를 끝내는 return문은 한번만 쓰자 라는 전통적인 패턴 )
	전통적인 패턴을 지키는 코드 예제 
	*/
	// Integer sum = null;
	// if((num1 + num2) == 0) {
	 ...
	 * 		addNum(int num1, int num2) 형식으로 method를 선언해야한다
	 * 		addNum(long num1, long num2)
	 * 만약 addNum(실수, 실수) 방식으로 호출하려면
	 * 		addNum(float num1, float num2) 형식으로 method를 선언해야한다.
	...
	public Float addNum(float num1, float num2) {
		...
		// method 코드가 아직 작성되지 않았을때 문법 오류가 나는 것을 방지하는 임시코드
		return null;
	}
	// 다형성
